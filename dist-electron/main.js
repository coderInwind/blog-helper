...
import require$$1, { ipcMain as ipcMain$1, app as app$1, BrowserWindow } from "electron";
...
function writeFile$1(file, obj, options, callback) {
  if (callback == null) {
    callback = options;
    options = {};
  }
...
function mkdirP(p, opts, f, made) {
  ...
  xfs.mkdir(p, mode, function(er) {
    ...
    switch (er.code) {
      case "ENOENT":
        if (path$7.dirname(p) === p)
          return cb(er);
        mkdirP(path$7.dirname(p), opts, function(er2, made2) {
          if (er2)
            cb(er2, made2);
          else
...
mkdirP.sync = function sync(p, opts, made) {
  ...
  try {
    ...
    switch (err0.code) {
      case "ENOENT":
        made = sync(path$7.dirname(p), opts, made);
        sync(p, opts, made);
        break;
      default:
        var stat;
...
var electronWindowState = function(options) {
  ...
  const fullStoreFileName = path$6.join(config.path, config.file);
  ...
  function saveState(win) {
    ...
    try {
      mkdirp.sync(path$6.dirname(fullStoreFileName));
...
const setUtilsListener = (manager) => {
  ...
  ipcMain$1.handle("maximize-window", () => {
    manager.zoom();
  });
  ipcMain$1.handle("close-window", () => {
    if (manager.hasWindow()) {
      manager.mainWindow.close();
    }
  });
  ipcMain$1.on("is-maximized", (event) => {
...
function setSettingsListener(manager) {
  ...
  ipcMain$1.handle("set-menu", (_, state) => {
    store.set(MENU_STORE_KEY, state);
  });
  ipcMain$1.on("get-menu", (event) => {
...
class WindowManager {
  constructor() {
    ...
    __publicField(this, "mainWindowState");
    this.init();
  }
  init() {
  ...
  createWindow() {
    this.mainWindow = new BrowserWindow({
      ...
      // 在mac端不隐藏navbar
      webPreferences: {
        preload: path$8.join(app$1.getAppPath(), "dist-electron/preload.mjs")
      }
    });
